    ...
    void Start()
    {
        if (isHorizontal)
        {
            for (int i = 0; i < 256; i++) //256 samples = half of the samples, which I am wrapping into a circle
            {
                ...
                instanceCube.transform.localPosition = Vector3.right * 10 * i;
    ...
    private void FixedUpdate()
    {
        for (int i = 0; i < 256; i++)
        {
            if (sampleCube != null)
            {
                ...
                sampleCube[i].transform.localScale = new Vector3(cubeScale, Mathf.Lerp(sampleCube[i].transform.localScale.y, temp, lerpSpeed), cubeScale);
            }
        }
    }
}
